...
function getHeatingDuration (date) {
	if (!date) {
		date = moment().tz("Europe/Bucharest");
	}

	return HeatingHistory
		.find({
			datetime: {
		...
		.then(results => {
			...
			if (lastEntry.status === true) {
				heatingDuration += (new Date(moment(date).tz("Europe/Bucharest").endOf('day')).getTime() - lastEntry.datetime.getTime()) / 60000;
			}
...
exports.getStatisticsForDate = function (date) {
	return Promise.all([
		getHeatingDuration(date)
	]).then(results => {
		return {
			heatingDuration: results[0]
		};
