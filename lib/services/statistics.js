...
function getHeatingDuration (date) {
	...
	return Promise.all([
		HeatingHistory
			.findOne({
				datetime: {
					$gt: moment(date).tz("Europe/Bucharest").subtract(1, 'days').startOf('day'),
					$lt: moment(date).tz("Europe/Bucharest").subtract(1, 'days').endOf('day')
				}
			})
			.sort({
				datetime: -1
		...
		results[0].datetime = moment(date).tz("Europe/Bucharest").startOf('day');
