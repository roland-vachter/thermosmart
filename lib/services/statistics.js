...
async function calculateHeatingDuration (date) {
	...
	const endDate = new Date(Math.min(new Date(moment().tz("Europe/Bucharest")), new Date(moment(date).tz("Europe/Bucharest").endOf('day'))));

	if (lastEntry.status === true) {
		console.log('belep', lastEntry, 'endDate', endDate);
...
/*exports.getStatisticsByMonth = async(dateStart, dateEnd) => {
	const heatingStatistics = await HeatingStatistics
		.find({
			date: {
				...
				$lt: new Date(moment(dateStart).tz('Europe/Bucharest').endOf('month'))
			}
		})
		.exec();
	...
	heatingStatistics.forEach(statistic => {
		if (currentMonth != moment(statistic.date).tz('Europe/Bucharest').startOf('month')) {
			avg.push({
				durationTotal: 0,
				targetTempTotal: 0,
				outsideTempTotal: 0,
