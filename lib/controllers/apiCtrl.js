	...
	Promise.all([
		...
		res.json({
			...
			data: {
				...
				statisticsForLastMonth: statisticsForLastMonth,
...
exports.statistics = async function (req, res) {
	const results = await statisticsService.getStatisticsByDay(moment(req.query.startDate).tz("Europe/Bucharest"), moment(req.query.endDate).tz("Europe/Bucharest"));
	res.json(results);
};
